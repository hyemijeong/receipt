...
import kotlinx.android.synthetic.main.fragment_ice_2.*

class ice_2 : Fragment() {
    ...
    override fun onCreateView(
        ...
        var rootView = inflater.inflate(R.layout.fragment_ice_2, container, false)
        val r = Runnable {
            try {
                val displayList = ArrayList<MyList>()
                mylistDb = MyListDb.getInstance(requireContext())

                myList= mylistDb?.myListDao()?.getIce()!!// mylist에 있는 내용 다 들고옴
                //데이터 읽고 쓸 떄는 쓰레드 사용
                Log.d("myList", "${myList.size}")
                displayList.addAll(myList)
                recyclerView1 = rootView.findViewById(R.id.rvMylist_ice!!)as RecyclerView
                val mylistAdapter = MyListAdapter(requireContext(), displayList)

                rvMylist_ice.adapter = mylistAdapter
                rvMylist_ice.layoutManager = LinearLayoutManager(requireContext())
